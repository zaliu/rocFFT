...
public:
    ...
    bool is_complex()
    {
        return _layout == rocfft_array_type_complex_interleaved || _layout == rocfft_array_type_complex_planar;
    }

    /*****************************************************/
    bool is_hermitian()
    {
        return _layout == rocfft_array_type_hermitian_interleaved || _layout == rocfft_array_type_hermitian_planar;
    }

    /*****************************************************/
    bool is_planar()
    {
        return _layout == rocfft_array_type_complex_planar || _layout == rocfft_array_type_hermitian_planar;
    }

    /*****************************************************/
    bool is_interleaved()
    {
        return _layout == rocfft_array_type_complex_interleaved || _layout == rocfft_array_type_hermitian_interleaved;
    }

    /*****************************************************/
    bool is_in_place()
    {
        if( _placeness ==  rocfft_placement_inplace ) return true;
        else if( _placeness ==  rocfft_placement_notinplace) return false;
                ...
                for( size_t y = 0; y < length(dimy); y++ )
                {
                    ...
                    for( size_t x = 0; x < number_of_points_in_one_period; x++) {
                        ...
                        else
                        {
                            ...
                            // if( x % 2 == 0 ) value *= -1;// change the sign, for debug
			                T imag = -2.0f * value;
		                    //if ( x==0 || x == number_of_points_in_one_period-1) 
