...
using namespace std;
    ...
    void print_matrix(vector<T> CPU_result, vector<T> GPU_result, size_t m, size_t n, size_t lda){
        for(int i=0;i<m;i++)
            for(int j=0;j<n;j++)
            {
                printf("matrix  col %d, row %d, CPU result=%f, GPU result=%f\n", i, j, (CPU_result[j+i*lda]).x, (GPU_result[j+i*lda]).x);
                printf("matrix  col %d, row %d, CPU result=%f, GPU result=%f\n", i, j, (CPU_result[j+i*lda]).y, (GPU_result[j+i*lda]).x);
            }
    }
...
template<class T>
rocfft_status rocfft_transpose(size_t m, size_t n, T *A, size_t lda, T *B, size_t ldb, size_t batch_count);
...
rocfft_status testing_transpose(size_t M, size_t N, size_t lda, size_t ldb, size_t batch_count)
{
    ...
    HIP_V_THROW(hipMemcpy(dA, hA.data(), sizeof(T) * A_size, hipMemcpyHostToDevice), "hipMemcpy failed");
    ...
    HIP_V_THROW(hipMemcpy(hB.data(), dB, sizeof(T) * B_size, hipMemcpyDeviceToHost), "hipMemcpy failed");
