...
#define TWIDDLE_3STEP_MUL_FWD(TWFUNC, INDEX, REG) \
	{ \
		float2 W = TWFUNC(INDEX); \
		float TR, TI; \
...
#define TWIDDLE_3STEP_MUL_INV(TWFUNC, INDEX, REG) \
	{ \
		float2 W = TWFUNC(INDEX); \
		float TR, TI; \
		TR =  (W.x * REG.x) + (W.y * REG.y); \
...
#define TWIDDLE_MUL_FWD(TWTABLE, INDEX, REG) \
	{ \
		float2 W = TWTABLE[INDEX]; \
		float TR, TI; \
		TR = (W.x * REG.x) - (W.y * REG.y); \
		TI = (W.y * REG.x) + (W.x * REG.y); \
		REG.x = TR; \
...
#define TWIDDLE_MUL_INV(TWTABLE, INDEX, REG) \
	{ \
		float2 W = TWTABLE[INDEX]; \
		float TR, TI; \
		TR =  (W.x * REG.x) + (W.y * REG.y); \
