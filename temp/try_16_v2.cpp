...
__device__ const float2 *gettw()
{
	const static float twiddles[] = {
...
__global__ void fft_fwd(hipLaunchParm lp, float2 *gb)
{
	...
	FwdPass1(me, 0, 0, lb, &R0, &R1, &R2, &R3);
}
...
//	hipMemcpyToSymbol((const char *)twiddles, twiddles_h, N * sizeof(float2), 0, hipMemcpyHostToDevice);
	...
	for(size_t p=0; p<10; p++)
	{
		...
		t.Start();
		// Launch HIP kernel
		hipLaunchKernel(HIP_KERNEL_NAME(fft_fwd), dim3(blocks), dim3(threadsPerBlock), 0, 0, x);
