...
#define SR_COMP_BOTH 2 // real & imag
                        ...
                        size_t regC, size_t numB, size_t numPrev, std::string &passStr, bool isPrecallVector = false, bool oddt = false) const
        {
            ...
            if( numB && (numB%2 == 0) && (regC == 1) && (numButterfly%2 == 0) && (algLS%2 == 0) && (flag == SR_WRITE) &&
                (nextPass == NULL) && interleaved && (component == SR_COMP_BOTH) && linearRegs && enableGrouping && !fft_doPostCallback )
            {
                ...
                passStr += " *buff4g = "; passStr += "("+ RegBaseType<PR>(4) + "*)" + bufferRe; passStr += ";\n\t"; // Assuming 'outOffset' is 0, so not adding it here
                ...
                for(size_t r=0; r<radix; r++)
                {
                    for(size_t i=0; i<numB; i++)
                    {
                        for(size_t c=cStart; c<cEnd; c++) // component loop: 0 - real, 1 - imaginary
                        {
                            ...
                            //get offset
                            bufOffset.clear();
                            bufOffset += offset; bufOffset += " + ( "; bufOffset += std::to_string(numPrev); bufOffset += " + ";
                            bufOffset += "me*"; bufOffset += std::to_string(numButterfly); bufOffset += " + ";
                            bufOffset += std::to_string(i); bufOffset += " + ";
                            bufOffset += std::to_string(r*length/radix); bufOffset += " )";
                ...
                for(size_t r=0; r<radix; r++)
                {
                    ...
                    for(size_t i=0; i<numB; i++)
                    {
                        ...
                        for(size_t c=cStart; c<cEnd; c++) // component loop: 0 - real, 1 - imaginary
                        {
                            ...
                            {
                                bufOffset += "(("; bufOffset += std::to_string(numButterfly);
                                bufOffset += "*me + "; bufOffset += std::to_string(butterflyIndex); bufOffset += ")/";
                                bufOffset += std::to_string(algLS); bufOffset += ")*"; bufOffset += std::to_string(algL); bufOffset += " + (";
                                bufOffset += std::to_string(numButterfly); bufOffset += "*me + "; bufOffset += std::to_string(butterflyIndex);
                                bufOffset += ")%"; bufOffset += std::to_string(algLS); bufOffset += " + ";
                            }
                            else
                            {
                                bufOffset += std::to_string(numButterfly); bufOffset += "*me + "; bufOffset += std::to_string(butterflyIndex);
                            ...
                            bufOffset += std::to_string(r*algLS); bufOffset += " )";
                    ...
                    for(size_t r=0; r<radix; r++)
                    {
                        for(size_t c=cStart; c<cEnd; c++) // component loop: 0 - real, 1 - imaginary
                        {
                            ...
                            for(size_t v=0; v<regC; v++) // TODO: vectorize the reads; instead of reading individually for consecutive reads of vector elements
                            {
                                ...
                                bufOffset += offset; bufOffset += " + ( "; bufOffset += std::to_string(numPrev); bufOffset += " + ";
                                bufOffset += "me*"; bufOffset += std::to_string(numButterfly); bufOffset += " + ";
                                bufOffset += std::to_string(i*regC + v); bufOffset += " + ";
                                bufOffset += std::to_string(r*length/radix); bufOffset += " )";
                                bufOffset +=  (stride == 1) ? " " : "*stride_in";
                ...
                else if( (flag == SR_TWMUL) || (flag == SR_TWMUL_3STEP) ) // twiddle multiplies and writes require that 'r' loop be innermost
                {
                    for(size_t v=0; v<regC; v++)
                    {
                        for(size_t r=0; r<radix; r++)
                        {
                            ...
                            if(regC != 1)
                            {
                                regRealIndex += ".s"; regRealIndex += std::to_string(v);
                                regImagIndex += ".s"; regImagIndex += std::to_string(v);
                            }
                            ...
                            if(flag == SR_TWMUL) // twiddle multiply operation
                            {
                                ...
                                passStr += twTable; passStr += "["; passStr += std::to_string(algLS-1); passStr += " + ";
                                passStr += std::to_string(radix-1); passStr += "*(("; passStr += std::to_string(numButterfly);
                                passStr += "*me + "; passStr += std::to_string(butterflyIndex); passStr += ")%";
                            ...
                            else    // 2, 3, or 4-step twiddle
                            {
                                ...
                                {
                                    assert(linearRegs);
                                    passStr += "("; passStr += "me*"; passStr += std::to_string(numButterfly);
                                    passStr += " + "; passStr += std::to_string(i); passStr += " + ";
                                    passStr += std::to_string(r*length/radix); passStr += ") * b";
                                }
                                else
                                {
                                    passStr += "(("; passStr += std::to_string(numButterfly); passStr += "*me + ";
                                    passStr += std::to_string(butterflyIndex);
                                    passStr += ")%"; passStr += std::to_string(algLS); passStr += " + ";
                                    passStr += std::to_string(r*algLS); passStr += ") * b";
                                }
            ...
            for(size_t r=rStart; r<rEnd; r++)
            {
                ...
                for(size_t c=cStart; c<cEnd; c++) // component loop: 0 - real, 1 - imaginary
                {
                    if(flag == SR_READ) // read operation
                    {
                        ...
                        {
                            idxStr += std::to_string(bid); idxStr += "*me +"; idxStr += oddpadd; idxStr += std::to_string(lid);
                        }
                        else
                        {
                            idxStr += "me + "; idxStr += std::to_string(1 + length*(r%bid)/numCR); idxStr += oddpadd;
                        }
                        idxStrRev += std::to_string(length); idxStrRev += " - ("; idxStrRev += idxStr; idxStrRev += " )";
                    ...
                    else // write operation
                    {
                        ...
                        if(!cid)
                        {
                            ...
                            if(fwd)
                            {
                                ...
                                if((length <= 2) || ((length & (length - 1)) != 0))
                                {
                                idxStr += std::to_string(length/(2*workGroupSize)); idxStr += "*me +"; idxStr += oddpadd; idxStr += std::to_string(lid);
                                }
                                else
                                {
                                idxStr += "me + "; idxStr += std::to_string(1 + length*(r%bid)/numCR); idxStr += oddpadd;
    ...
    public:
                            ...
                            bool gIn = false, bool gOut = false) const
        {
            ...
            if(r2c || c2r)
            {
                ...
                if(gIn)
                {
                    if(inInterleaved)
                    {
                        ...
                        if(!rcSimple) {     passStr += regB2Type; passStr += " *"; passStr += bufferInRe2; passStr += ", "; }
                    }
                    else if(inReal)
                    {
                                         passStr += regB1Type; passStr += " *"; passStr += bufferInRe;  passStr += ", ";
                        if(!rcSimple) {     passStr += regB1Type; passStr += " *"; passStr += bufferInRe2; passStr += ", "; }
                    }
                    else
                    {
                                         passStr += regB1Type; passStr += " *"; passStr += bufferInRe;  passStr += ", ";
                        ...
                        if(!rcSimple) {     passStr += regB1Type; passStr += " *"; passStr += bufferInIm2; passStr += ", "; }
                    }
                }
                else
                {
                ...
                if(gOut)
                {
                    if(outInterleaved)
                    {
                        ...
                        if(!rcSimple) { passStr += ", ";     passStr += regB2Type; passStr += " *"; passStr += bufferOutRe2; }
                    }
                    else if(outReal)
                    {
                                                             passStr += regB1Type; passStr += " *"; passStr += bufferOutRe;
                        if(!rcSimple) { passStr += ", ";     passStr += regB1Type; passStr += " *"; passStr += bufferOutRe2; }
                    }
                    else
                    {
                                                             passStr += regB1Type; passStr += " *"; passStr += bufferOutRe;  passStr += ", ";
                        if(!rcSimple) {                         passStr += regB1Type; passStr += " *"; passStr += bufferOutRe2; passStr += ", "; }
                                                             passStr += regB1Type; passStr += " *"; passStr += bufferOutIm;
                        if(!rcSimple) { passStr += ", ";     passStr += regB1Type; passStr += " *"; passStr += bufferOutIm2; }
                    }
                }
                else
                {
            ...
            if( !rcSimple && oddp && ((r2c && (nextPass == NULL)) || (c2r && (position == 0))) )
            {
                ...
                passStr += "\n\t";
                passStr += regB2Type; passStr += " R"; passStr += std::to_string(cnPerWI); passStr += "[1];\n\t";
                passStr += "(*R"; passStr += std::to_string(cnPerWI); passStr += ").x = 0; ";
                passStr += "(*R"; passStr += std::to_string(cnPerWI); passStr += ").y = 0;\n";
            }
            ...
            if( !rcSimple && (c2r && (position == 0)) && singlePass )
            {
                ...
                passStr += "\n\t";
                passStr += regB1Type;
                passStr += " mpvt["; passStr += std::to_string(length); passStr += "];\n";
            }
