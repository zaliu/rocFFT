...
namespace StockhamGenerator
{
	...
	inline std::string RegBaseType(size_t count)
	{
		switch(count)
			...
			case 2: return "T"; //float2, double2
			case 4: return "vector4_type_t<T>"; //float4, double4
	...
	inline std::string FloatSuffix()
	{
		...
		switch(PR)
		{
		case rocfft_precision_single: sfx = "f"; break;
		case rocfft_precision_double: sfx = "";  break;
	...
	inline std::string ButterflyName(size_t radix, size_t count, bool fwd)
	{
		...
		str += "Rad"; str += std::to_string(radix);
		str += "B"; str += std::to_string(count);
		return str;
	}
