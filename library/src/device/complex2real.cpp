...
template<typename T>
__global__
void complex2real_kernel(size_t input_size, size_t input_stride, size_t output_stride, T *input, size_t input_distance, real_type_t<T> *output, size_t output_distance)
{
    ...
    input_offset += hipBlockIdx_y * input_stride;//notice for 1D, hipBlockIdx_y == 0 and thus has no effect for input_offset
    output_offset += hipBlockIdx_y * output_stride;//notice for 1D, hipBlockIdx_y == 0 and thus has no effect for output_offset
...
void complex2real(const void *data_p, void *back_p) 
{
    ...
    if(precision == rocfft_precision_single) 
        hipLaunchKernelGGL( complex2real_kernel<float2>, grid, threads, 0, rocfft_stream, input_size, input_stride, output_stride, (float2 *)input_buffer, input_distance, (float *)output_buffer, output_distance);  
    else 
        hipLaunchKernelGGL( complex2real_kernel<double2>, grid, threads, 0, rocfft_stream, input_size, input_stride, output_stride, (double2 *)input_buffer, input_distance, (double *)output_buffer, output_distance);
...
template<typename T>
__global__
void hermitian2complex_kernel(size_t hermitian_size, size_t dim_0, size_t dim_1, size_t dim_2,
        size_t input_stride, size_t output_stride, T *input, size_t input_distance, T *output, size_t output_distance)
{
    size_t tid = hipBlockIdx_x * hipBlockDim_x + hipThreadIdx_x;
    ...
    input_offset += hipBlockIdx_y * input_stride + is0;//notice for 1D, hipBlockIdx_y == 0 and thus has no effect for input_offset
...
/*! \brief auxiliary function
    ...
    input_buffer
          ...
          but only store first [1 + dim_0/2] elements according to conjugate symmetry
...
void hermitian2complex(const void *data_p, void *back_p) 
{
    ...
    size_t j = 0;
    {
        ...
        { 
            printf("kernel input[%zu][%zu]=(%f, %f) \n", j, i, tmp[j*input_stride + i].x, tmp[j*input_stride + i].y);
        }
    }*/

    if(precision == rocfft_precision_single) 
        hipLaunchKernelGGL( hermitian2complex_kernel<float2>, grid, threads, 0, rocfft_stream, hermitian_size, dim_0, dim_1, dim_2, input_stride, output_stride, (float2 *)input_buffer, input_distance, (float2 *)output_buffer, output_distance);  
    else 
        hipLaunchKernelGGL( hermitian2complex_kernel<double2>, grid, threads, 0, rocfft_stream, hermitian_size, dim_0, dim_1, dim_2, input_stride, output_stride, (double2 *)input_buffer, input_distance, (double2 *)output_buffer, output_distance);  

    /*float2* tmpo; tmpo = (float2*)malloc(sizeof(float2)*output_distance*batch);
    hipMemcpy(tmpo, output_buffer, sizeof(float2)*output_distance*batch, hipMemcpyDeviceToHost);

    {
        for(size_t i=0; i<output_stride; i++)
        { 
            printf("kernel output[%zu][%zu]=(%f, %f) \n", j, i, tmpo[j*output_stride + i].x, tmpo[j*output_stride + i].y);
        }
    }*/
    return;    
}
