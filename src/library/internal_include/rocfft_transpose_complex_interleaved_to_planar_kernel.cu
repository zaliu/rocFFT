...
//works for complex interleaved to complex planar; T should be float2 or double2; T2 should be float or double
										...
										size_t batch_size)
{
	...
	__shared__ T2 lds_real[macro_tile_row_size][macro_tile_col_size];
	__shared__ T2 lds_imag[macro_tile_row_size][macro_tile_col_size];
	...
	for(int i = 0; i < unroll_factor; i++)
	{
		...
		lds_real[subblock_idx_0][subblock_idx_1] = input_matrix[subblock_idx_1 * input_leading_dim_size + subblock_idx_0].x;
		lds_imag[subblock_idx_0][subblock_idx_1] = input_matrix[subblock_idx_1 * input_leading_dim_size + subblock_idx_0].y;
	}
