...
{
  String build_image_name = "build-rocfft-hip-artifactory"
...
//    if( paths.project_name.equalsIgnoreCase( 'rocfft-ubuntu' ) )
//    {
//      stage('Clang Format')
//      {
//        sh '''
...
//    }
    ...
    stage( "Test ${compiler_args.compiler_name} ${compiler_args.build_config}" )
    {
      ...
      timeout(time: 2, unit: 'HOURS')
      {
        if(isJobStartedByTimer())
        {
          sh """#!/usr/bin/env bash
                ...
                LD_LIBRARY_PATH=/opt/rocm/hcc/lib ./rocfft-test${build_type_postfix} --gtest_output=xml --gtest_color=yes
            """
      ...
      if( compiler_args.compiler_name.toLowerCase( ).startsWith( 'hcc-' ) )
      {
        ...
        else if( paths.project_name.equalsIgnoreCase( 'rocfft-fedora' ) )
        {
          sh  """#!/usr/bin/env bash
              set -x
              rm -rf ${docker_context} && mkdir -p ${docker_context}
...
String hip_integration_testing( String inside_args, String job, String build_config )
{
  ...
  def rocfft_integration_image = null
  ...
  String rocfft_src_rel = checkout_and_version( job )

  // Conctruct a binary directory path based on build config
  String rocfft_bin_rel = build_directory_rel( build_config )

  // Build rocfft inside of the build environment
  rocfft_integration_image = docker_build_image( job, testing_org_name, '', rocfft_src_rel, "${testing_org_name}/${hip_test_image_name}" )

  docker_build_inside_image( rocfft_integration_image, inside_args, job, '', build_config, rocfft_src_rel, rocfft_bin_rel )
...
def build_pipeline( compiler_data compiler_args, docker_data docker_args, project_paths rocfft_paths, def docker_inside_closure )
{
  ansiColor( 'vga' )
  {
    stage( "Build ${compiler_args.compiler_name} ${compiler_args.build_config}" )
    {
      ...
      // Build rocfft inside of the build environment
      docker_build_inside_image( rocfft_build_image, compiler_args, docker_args, rocfft_paths )
    }

    if( !rocfft_paths.project_name.equalsIgnoreCase( 'rocfft-hcc-ctu' ) )
    {
